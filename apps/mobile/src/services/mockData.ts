...
// Seed data is identical so the first to load populates local storage
...
export const initializeMockDb = () => {
  ...
  getLocalStorage('cme_inspecoes', []);
  getLocalStorage('cme_current_user', SEED_USERS[1]); // Default to operator for mobile
};
...
export const getInspecoes = (): Inspecao[] => {
  initializeMockDb();
  const raw = getLocalStorage<Inspecao[]>('cme_inspecoes', []);
...
export const saveInspecao = (inspecao: Inspecao): void => {
  initializeMockDb();
  const current = getLocalStorage<Inspecao[]>('cme_inspecoes', []);
...
export const getLogisticaCurrentUser = (): User => {
  initializeMockDb();
  return getLocalStorage('cme_current_user', SEED_USERS[1]);
};
