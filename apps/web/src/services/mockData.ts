import { Equipamento, ChecklistModelo, Material, Inspecao, User } from '@cme/types';
...
const SEED_CHECKLIST_MODELOS: ChecklistModelo[] = [
  {
    id: 'mod-1',
    ...
    ativo: true,
    itens: [
      // 1. INSPEÇÃO DO CONTAINER E LINGADA
      ...
      { id: 'it-6', modeloId: 'mod-1', secao: 'INSPEÇÃO GERAL', ordem: 6, descricao: 'Conferir se todos os manômetros e vasos de pressão estão íntegos e identificados com adesivos de certificação e NR13', obrigatorio: true },
      ...
      // 3. INSPEÇÃO DO SISTEMA PNEUMÁTICO (DESPRESSURIZADO)
      ...
      // 4. INSPEÇÃO DO SISTEMA ELÉTRICO
      ...
      // 5. INSPEÇÃO DO SISTEMA PNEUMÁTICO (PRESSURIZADO)
      ...
      // 6. TESTE OPERACIONAL DO SISTEMA
...
const SEED_INSPECOES: Inspecao[] = [
  {
    id: 'insp-1',
    equipamentoId: 'eq-1',
    tipo: 'PRE_EMBARQUE',
    data: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(), // 3 dias atrás
    responsavelGeral: 'Pedro Souza',
    localizacao: 'Oficina Macaé',
    status: 'VALIDADA',
    observacoesGerais: 'Equipamento em excelentes condições de operação. Liberado para embarque imediato.',
    createdById: 'usr-2',
    respostas: [
      { id: 'resp-1', inspecaoId: 'insp-1', itemId: 'it-1', status: 'OK', observacao: 'Certificado Válido até 12/2026' },
      { id: 'resp-2', inspecaoId: 'insp-1', itemId: 'it-2', status: 'OK', observacao: 'Certificado Válido até 08/2026' },
      { id: 'resp-3', inspecaoId: 'insp-1', itemId: 'it-3', status: 'OK' },
      { id: 'resp-4', inspecaoId: 'insp-1', itemId: 'it-4', status: 'OK' },
      { id: 'resp-5', inspecaoId: 'insp-1', itemId: 'it-5', status: 'OK', responsavel: 'Mecânica' },
      { id: 'resp-6', inspecaoId: 'insp-1', itemId: 'it-6', status: 'OK' },
      { id: 'resp-7', inspecaoId: 'insp-1', itemId: 'it-7', status: 'OK' },
      { id: 'resp-8', inspecaoId: 'insp-1', itemId: 'it-8', status: 'OK', responsavel: 'Elétrica' },
      { id: 'resp-9', inspecaoId: 'insp-1', itemId: 'it-9', status: 'OK' },
      { id: 'resp-10', inspecaoId: 'insp-1', itemId: 'it-10', status: 'OK' },
    ],
    materiais: [
      { id: 'mu-1', inspecaoId: 'insp-1', materialId: 'mat-1', quantidade: 1, observacao: 'Substituição preventiva' }
    ]
  },
  {
    id: 'insp-2',
    ...
    responsavelGeral: 'Pedro Souza',
    localizacao: 'Plataforma P-58',
    status: 'EM_ANDAMENTO',
    observacoesGerais: 'Aguardando chegada do disjuntor reserva para finalizar o painel.',
    createdById: 'usr-2',
    respostas: [
      { id: 'resp-20', inspecaoId: 'insp-2', itemId: 'it-1', status: 'OK' },
      { id: 'resp-21', inspecaoId: 'insp-2', itemId: 'it-2', status: 'OK' },
      { id: 'resp-22', inspecaoId: 'insp-2', itemId: 'it-3', status: 'OK' },
      { id: 'resp-23', inspecaoId: 'insp-2', itemId: 'it-8', status: 'PENDENTE', observacao: 'Falta disjuntor de comando no painel', responsavel: 'Elétrica' },
    ],
    materiais: []
  }
];
...
export const initializeMockDb = () => {
  getLocalStorage('cme_equipamentos', SEED_EQUIPAMENTOS);
  getLocalStorage('cme_checklist_modelos', SEED_CHECKLIST_MODELOS);
  getLocalStorage('cme_materiais', SEED_MATERIAIS);
  getLocalStorage('cme_users', SEED_USERS);
  getLocalStorage('cme_inspecoes', SEED_INSPECOES);
  getLocalStorage('cme_current_user', SEED_USERS[0]); // Default manager
};

export const getEquipamentos = (): Equipamento[] => {
  initializeMockDb();
  return getLocalStorage('cme_equipamentos', SEED_EQUIPAMENTOS);
};

export const getChecklistModelos = (): ChecklistModelo[] => {
  initializeMockDb();
  return getLocalStorage('cme_checklist_modelos', SEED_CHECKLIST_MODELOS);
};

export const getMateriais = (): Material[] => {
  initializeMockDb();
  return getLocalStorage('cme_materiais', SEED_MATERIAIS);
};

export const getUsers = (): User[] => {
  initializeMockDb();
  return getLocalStorage('cme_users', SEED_USERS);
};

export const getInspecoes = (): Inspecao[] => {
  initializeMockDb();
  const raw = getLocalStorage('cme_inspecoes', SEED_INSPECOES);
  // Re-link nested structures
  const equipments = getEquipamentos();
  const templates = getChecklistModelos();
  const materials = getMateriais();
  
  return raw.map(insp => {
    const eq = equipments.find(e => e.id === insp.equipamentoId);
    
    const linkedRespostas = insp.respostas.map(resp => {
      const template = templates[0]; // After Cooler template
      const item = template.itens?.find(i => i.id === resp.itemId);
      return { ...resp, item };
    });

    const linkedMateriais = insp.materiais.map(mat => {
    ...
    return {
      ...insp,
      equipamento: eq,
      respostas: linkedRespostas,
      materiais: linkedMateriais
    };
  });
};

export const saveInspecao = (inspecao: Inspecao): void => {
  initializeMockDb();
  const current = getLocalStorage<Inspecao[]>('cme_inspecoes', SEED_INSPECOES);
  
  // Clean circular/joined fields before saving to store plain ids
  const cleanInspecao = {
    ...inspecao,
    equipamento: undefined,
    respostas: inspecao.respostas.map(r => ({ ...r, item: undefined })),
    materiais: inspecao.materiais.map(m => ({ ...m, material: undefined }))
  };

  const idx = current.findIndex(i => i.id === inspecao.id);
  if (idx !== -1) {
    current[idx] = cleanInspecao;
  } else {
    current.unshift(cleanInspecao);
  }
  
  setLocalStorage('cme_inspecoes', current);
};

export const getInspecaoById = (id: string): Inspecao | undefined => {
  return getInspecoes().find(i => i.id === id);
};

export const getChecklistModeloPorTipo = (tipo: string): ChecklistModelo | undefined => {
  return getChecklistModelos().find(m => m.tipoEquipamento === tipo);
};

export const getLogisticaCurrentUser = (): User => {
  initializeMockDb();
  return getLocalStorage('cme_current_user', SEED_USERS[0]);
};

export const setLogisticaCurrentUser = (user: User): void => {
  setLocalStorage('cme_current_user', user);
};
